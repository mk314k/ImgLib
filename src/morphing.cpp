...
Vect2f operator*(const Vect2f &a, float f) {
  // Return a*f
  return Vect2f(f*a.x, f*a.y); 
}

Vect2f operator/(const Vect2f &a, float f) {
  // Return a/f
  return Vect2f(a.x/f, a.y/f); 
}
...
vector<Image> morph(const Image &im_before, const Image &im_after,
  ...
  for (int i=1; i<=N;i++){
    ...
    imgHandle.imwrite(img1, "../Output/img1morph.png");
    imgHandle.imwrite(img2, "../Output/img2morph.png");
    for (int x=0; x<img.width();x++){
      for (int y=0;y<img.height();y++){
        for (int z=0; z<img.channels(); z++){
